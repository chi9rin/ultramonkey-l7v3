...
$L7VSADM -A -t 127.0.0.1:40001 -m ip -f 1
if [ $? -ne 0 ]
then
...
$L7VSADM -E -t 127.0.0.1:40001 -m ip -s wrr
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -E -t 127.0.0.1:40001 -m ip -s wrr"
        exit 1
fi
...
Prot LocalAddress:Port ProtoMod Scheduler Protomod_opt_string
  -> RemoteAddress:Port           Forward Weight ActiveConn InactConn
TCP 127.0.0.1:40001 ip wrr --timeout 3600 --no-reschedule --sorry-uri '/' --statistic 0
  Bypass Settings:
    Sorry Server                  none
    Max Connection                0
    Sorry Flag                    on

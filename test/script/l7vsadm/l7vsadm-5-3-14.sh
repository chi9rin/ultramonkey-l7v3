...
$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer3_ADDR}:${RealServer3_PORT} --tproxy
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer3_ADDR}:${RealServer3_PORT} --tproxy"
        exit 1
fi

$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer4_ADDR}:${RealServer4_PORT} --tproxy
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer4_ADDR}:${RealServer4_PORT} --tproxy"
        exit 1
fi

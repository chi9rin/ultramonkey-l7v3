...
RET=`$L7VSADM -d -t 333.333.333.333:3333 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} 2>&1 | grep "PARSE ERROR : target endpoint parse error:invalid endpoint:Host not found (non-authoritative), try again later:333.333.333.333:3333"`
...
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -d -t 333.333.333.333:3333 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT}"
        exit 1
fi

...
$L7VSADM -A -t localhost:40001 -m ip -Q 300M
if [ $? -ne 0 ]
then        
        echo "Test failed: $L7VSADM -A -t localhost:40001 -m ip -Q 300M"
        exit 1
fi

$L7VSADM -E -t localhost:40001 -m ip -Q 0M
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -E -t localhost:40001 -m ip -Q 0M"
        exit 1
fi

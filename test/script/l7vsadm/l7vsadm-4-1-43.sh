...
$L7VSADM -A -t 127.0.0.1:40001 -m ip -L 1 -a ${L7VS_LOG_DIR}/access_virtualservice.log
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -A -t 127.0.0.1:40001 -m ip -L 1 -a ${L7VS_LOG_DIR}/access_virtualservice.log"
        exit 1
fi
...
TCP 127.0.0.1:40001 ip rr --timeout 3600 --no-reschedule --sorry-uri '/' --statistic 0
  ...
  Logging Settings:
    Access Log                    on
    Access Log File               ${L7VS_LOG_DIR}/access_virtualservice.log

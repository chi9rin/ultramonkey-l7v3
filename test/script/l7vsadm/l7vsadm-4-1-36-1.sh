...
$L7VSADM -A -t 127.0.0.1:40001 -m ip -O deferaccept,nodelay,cork,quickackon
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -A -t 127.0.0.1:40001 -m ip -O deferaccept,nodelay,cork,quickackon"
        exit 1
fi
...
TCP 127.0.0.1:40001 ip rr --timeout 3600 --no-reschedule --sorry-uri '/'
    ...
    0 0
    none
    deferaccept,nodelay,cork,quickackon
    0
    none

...
RET=`$L7VSADM -e -t :3333 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -w 2 2>&1 | grep "PARSE ERROR: target endpoint parse error(--tcp-service): hostname is not specified::3333"`
EXPECT="PARSE ERROR: target endpoint parse error(--tcp-service): hostname is not specified::3333"
echo "$RET"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -e -t :3333 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -w 2"
        exit 1
fi

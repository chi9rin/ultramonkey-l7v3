...
RET=`$L7VSADM -e -t 127.0.0.1:40001 -m ip -r 0.0.0.0:3333 -w 3 2>&1 | grep "PARSE ERROR: realserver endpoint parse error(--real-server): invalid address (INADDR_ANY):0.0.0.0:3333"`
EXPECT="PARSE ERROR: realserver endpoint parse error(--real-server): invalid address (INADDR_ANY):0.0.0.0:3333"

if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r 0.0.0.0:3333 -w 3"
        exit 1
fi

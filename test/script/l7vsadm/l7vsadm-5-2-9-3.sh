...
RET=`$L7VSADM -d -t 127.0.0.1:40001 -m ip -r :3333 2>&1 | grep "PARSE ERROR: realserver endpoint parse error(--real-server): hostname is not specified::3333"`
EXPECT="PARSE ERROR: realserver endpoint parse error(--real-server): hostname is not specified::3333"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -d -t 127.0.0.1:40001 -m ip -r :3333"
        exit 1
fi

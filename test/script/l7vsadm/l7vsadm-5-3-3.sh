...
RET=`$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -x  2>&1 | grep "PARSE ERROR: realserver option not found(--real-server): -x"`
EXPECT="PARSE ERROR: realserver option not found(--real-server): -x"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -x"
        exit 1
fi

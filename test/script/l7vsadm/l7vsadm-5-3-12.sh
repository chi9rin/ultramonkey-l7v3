...
$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -w 0
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -w 0"
        exit 1
fi

$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer2_ADDR}:${RealServer2_PORT} -w 0
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer2_ADDR}:${RealServer2_PORT} -w 0"
        exit 1
fi

$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer3_ADDR}:${RealServer3_PORT} -w 0
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer3_ADDR}:${RealServer3_PORT} -w 0"
        exit 1
fi

$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer4_ADDR}:${RealServer4_PORT} -w 0
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer4_ADDR}:${RealServer4_PORT} -w 0"
        exit 1
fi
...
TCP 127.0.0.1:40001 ip rr
  -> ${RealServer1_ADDR}:${RealServer1_PORT}              Masq    0      0          0         
  -> ${RealServer2_ADDR}:${RealServer2_PORT}                  Masq    0      0          0         
  -> ${RealServer3_ADDR}:${RealServer3_PORT}              Tproxy  0      0          0         
  -> ${RealServer4_ADDR}:${RealServer4_PORT}                  Tproxy  0      0          0         "
if [ "${RET}" != "${EXPECT}" ]
then

...
RET=`$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -w -1 2>&1 | grep "PARSE ERROR: invalid weight value.(--weight)"`
EXPECT="PARSE ERROR: invalid weight value.(--weight)"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT} -w -1"
        exit 1
fi

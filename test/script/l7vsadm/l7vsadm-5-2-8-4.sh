...
RET=`$L7VSADM -d -t 127.0.0.1:40001 -m x -r ${RealServer1_ADDR}:${RealServer1_PORT} 2>&1 | grep "PARSE ERROR: protocol module not found(--proto-module): x"`
EXPECT="PARSE ERROR: protocol module not found(--proto-module): x"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -d -t 127.0.0.1:40001 -m x -r ${RealServer1_ADDR}:${RealServer1_PORT}"
        exit 1
fi

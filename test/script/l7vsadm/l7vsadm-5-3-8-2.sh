...
RET=`$L7VSADM -e -t 127.0.0.1:40001 -m rrrrrrrrrrrrrrrrrrrr -r ${RealServer1_ADDR}:${RealServer1_PORT} -w 3 2>&1 | grep "PARSE ERROR: protocol module name is too long.(--proto-module)"`
EXPECT="PARSE ERROR: protocol module name is too long.(--proto-module)"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m rrrrrrrrrrrrrrrrrrrr -r ${RealServer1_ADDR}:${RealServer1_PORT} -w 3"
        exit 1
fi

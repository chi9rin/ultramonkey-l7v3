...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/virtualservice-138-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
RET=`cat ${L7VS_LOG_DIR}/l7vsd.log | grep "\[ERROR\] l7vsd_virtualservice VSD40700034 Invalid session_cache parameter value."`
if [ -z "${RET}" ]
then

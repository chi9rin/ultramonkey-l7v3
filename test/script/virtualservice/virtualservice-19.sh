...
RET=`$L7VSADM -d -t 127.0.0.1:40001 -m ip -r ${RealServer2_ADDR}:${RealServer2_PORT} 2>&1`
EXPECT="del rs error : Real Server does not exist."
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -d -t 127.0.0.1:40001 -m ip -r ${RealServer2_ADDR}:${RealServer2_PORT}"
        exit 1
fi

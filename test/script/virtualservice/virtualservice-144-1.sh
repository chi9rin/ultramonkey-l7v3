...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/virtualservice-144-1-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
\cp -f materials/virtualservice-144-1-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
RET=`cat ${L7VS_LOG_DIR}/l7vsd.log | grep "\[ERROR\] l7vsd_virtualservice VSD40700036 Invalid session_cache_timeout parameter value."`
if [ -z "${RET}" ]
then

...
$L7VSADM -A -t localhost:40001 -m sessionless -s wrr -u 10 -b ${SorryServer1_ADDR}:${SorryServer1_PORT} -f 0 -Q 100M -q 100M -O deferaccept,cork,quickackoff,nodelay -L 1 -a /var/log/l7vs/l7vsd_conn.log --ac-rotate-type size --ac-rotate-max-backup-index 12 --ac-rotate-max-filesize 1M
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -A -t localhost:40001 -m sessionless -s wrr -u 10 -b ${SorryServer1_ADDR}:${SorryServer1_PORT} -f 0 -Q 100M -q 100M -O deferaccept,cork,quickackoff,nodelay -L 1 -a /var/log/l7vs/l7vsd_conn.log --ac-rotate-type size --ac-rotate-max-backup-index 12 --ac-rotate-max-filesize 1M"
        exit 1
fi

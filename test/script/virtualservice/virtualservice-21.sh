...
RET=`$L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer2_ADDR}:${RealServer2_PORT} -w 2 2>&1`
EXPECT="edit rs error : Real Server does not exist."

if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m ip -r ${RealServer2_ADDR}:${RealServer2_PORT} -w 2"
        exit 1
fi

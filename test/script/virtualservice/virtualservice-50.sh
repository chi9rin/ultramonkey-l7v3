...
$L7VSADM -A -t 127.0.0.1:40002 -m ip
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -A -t 127.0.0.1:40002 -m ip"
        exit 1
fi

$L7VSADM -E -t 127.0.0.1:40001 -m ip -Q 100K -q 100K
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -E -t 127.0.0.1:40001 -m ip -Q 100K -q 100K"
        exit 1
fi

$L7VSADM -E -t 127.0.0.1:40002 -m ip -Q 100M -q 100M
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -E -t 127.0.0.1:40002 -m ip -Q 100M -q 100M"
        exit 1
fi
...
TCP 127.0.0.1:40001 ip rr --timeout 3600 --no-reschedule --sorry-uri '/' --statistic 0
  ...
  Socket Settings:
    ...
    TCP_QUICKACK                  auto
  Throughput:
    Current Upload / Limit        0.000000 Mbps / 0.100000 Mbps
    Current Download / Limit      0.000000 Mbps / 0.100000 Mbps
  Statistics:

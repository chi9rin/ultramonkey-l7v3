...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/virtualservice-136-2-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
RET=`cat ${L7VS_LOG_DIR}/l7vsd.log | grep "\[WARN\] l7vsd_virtualservice VSD30700019 session_cache parameter not found. Use default value."`
if [ -z "${RET}" ]
then

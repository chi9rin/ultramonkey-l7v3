#!/bin/bash
. ${SET_DEFAULT_CONF}
\cp ./materials/l7directord-5-2-26-a-l7directord.cf ${L7DIRECTORD_CONF_DIR}/l7directord.cf
...
start_lighttpd -s $RealServer1 -a $RealServer1_ADDR -p $RealServer1_PORT -l ${L7VS_LOG_DIR}/l7directord-5-2-26-real1a-access.log
if [ $? -ne 0 ]
then
        echo "Test failed: start_lighttpd RealServer1"
        exit 1
...
RET=`cat ${L7VS_LOG_DIR}/l7directord-5-2-26-real1a-access.log`
if [ -z "$RET" ]
then
        echo "Test failed: ${L7VS_LOG_DIR}/l7directord-5-2-26-real1a-access.log"
        exit 1
fi
...
TIME1=""
TIME2=""
for TMP in `cat ${L7VS_LOG_DIR}/l7directord-5-2-26-real1a-access.log | awk '{ print $4 }' | sed 's/:/ /' | awk '{ print $2}'`
do
        TIME2=`date -d "$TMP" +'%s'`
        if [ -n "$TIME1" ]
        then
...
\cp ./materials/l7directord-5-2-26-b-l7directord.cf ${L7DIRECTORD_CONF_DIR}/l7directord.cf
...
start_lighttpd -s $RealServer1 -a $RealServer1_ADDR -p $RealServer1_PORT -l ${L7VS_LOG_DIR}/l7directord-5-2-26-real1b-access.log
if [ $? -ne 0 ]
then
        echo "Test failed: start_lighttpd RealServer1"
        exit 1
...
RET=`cat ${L7VS_LOG_DIR}/l7directord-5-2-26-real1b-access.log`
if [ -z "$RET" ]
then
        echo "Test failed: ${L7VS_LOG_DIR}/l7directord-5-2-26-real1b-access.log"
        exit 1
fi
...
TIME1=""
TIME2=""
for TMP in `cat ${L7VS_LOG_DIR}/l7directord-5-2-26-real1b-access.log | awk '{ print $4 }' | sed 's/:/ /' | awk '{ print $2}'`
do
        TIME2=`date -d "$TMP" +'%s'`
        if [ -n "$TIME1" ]
        then

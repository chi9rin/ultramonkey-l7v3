...
#Restart l7directord
$L7DIRECTORD ${L7DIRECTORD_CONF_DIR}/specified.cf restart
if [ $? -ne 0 ]
then
        echo "Test failed: $L7DIRECTORD ${L7DIRECTORD_CONF_DIR}/specified.cf restart"
        exit 1
fi
usleep 100000


RET=`ps -ef | grep "l7directord ${L7DIRECTORD_CONF_DIR}/specified.cf restart"| grep -v grep`
if [ -z "$RET" ]
then
        echo "Test failed: ps -ef | grep l7directord ${L7DIRECTORD_CONF_DIR}/specified.cf restart | grep -v grep"
        exit 1
fi

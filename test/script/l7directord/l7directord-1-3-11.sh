#!/bin/bash
. ${SET_DEFAULT_CONF}
\cp ./materials/l7directord-1-3-11-l7directord.cf ${L7DIRECTORD_CONF_DIR}/l7directord.cf
...
kill -XFSZ `ps -ef | grep "$L7DIRECTORD" | grep -v grep | awk '{print $2}'`
if [ $? -ne 0 ]
then
        echo "Test failed: kill -XFSZ \`ps -ef | grep "$L7DIRECTORD" | grep -v grep | awk '{print \$2}'\`"
        exit 1
fi
sleep 1

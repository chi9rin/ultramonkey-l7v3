#!/bin/bash
. ${SET_DEFAULT_CONF}
\cp ./materials/l7directord-4-4-17-l7directord.cf ${L7DIRECTORD_CONF_DIR}/l7directord.cf

# Run server (sleep 0 sec. 200 OK)
PATH=$PATH:$COMMON_SCRIPT_DIR
test_server -p 20000 ./materials/l7directord-4-4-17-response.txt &
...
if [ $? -ne 0 ]; then
	pkill test_server
        echo "Test failed: Cannot find health check timeout log"
        exit 1
fi

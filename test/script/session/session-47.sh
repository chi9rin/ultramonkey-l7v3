...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-47-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
...
RET=`wget -qO- https://127.0.0.1:40001/ --no-check-certificate --secure-protocol=TLSv1`
EXPECT="${RealServer1}"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: wget -qO- https://127.0.0.1:40001/ --no-check-certificate --secure-protocol=TLSv1"
        exit 1
fi

...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-66-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
\cp -f materials/session-66-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
openssl s_client -host 127.0.0.1 -port 40001 -cipher 'ALL:!LOW'
if [ $? -eq 0 ]
then
        echo "Test failed: penssl s_client -host 127.0.0.1 -port 40001 -cipher 'ALL:!LOW'"
        exit 1
fi

...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-48-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
...
RET=`wget -qO- -t 1 -T 1 https://127.0.0.1:40001/ --no-check-certificate --secure-protocol=SSLv2`
if [ $? -eq 0 ]
then
        echo "Test failed: wget -qO- -t 1 -T 1 https://127.0.0.1:40001/ --no-check-certificate --secure-protocol=SSLv2"
        exit 1
fi
...
RET=`cat ${L7VS_LOG_DIR}/l7vsd.log | egrep "\[DEBUG\] l7vsd_session VSD10900071 Thread ID\[.*\] tcp_ssl_socket::handshake \[asio.ssl error\]"`
if [ -z "${RET}" ]
then

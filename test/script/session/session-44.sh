...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-44-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
\cp -f materials/session-44-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
RET=`cat ${L7VS_LOG_DIR}/l7vsd.log | egrep "\[DEBUG\] l7vsd_session VSD10900070 Thread ID\[.*\] ssl session handshaking start: set handshake timer 33sec"`
if [ -z "${RET}" ]
then

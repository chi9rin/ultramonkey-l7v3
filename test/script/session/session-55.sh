...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-55-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
\cp -f materials/session-55-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
mkdir -p ${L7VSD_CONF_DIR}/sslproxy/cert/
\cp -f materials/session-55-server.crt ${L7VSD_CONF_DIR}/sslproxy/cert/server.crt
\cp -f materials/session-55-newkey.pem ${L7VSD_CONF_DIR}/sslproxy/cert/newkey.pem
\cp -f materials/session-55-passwd.txt ${L7VSD_CONF_DIR}/sslproxy/passwd.txt
\cp -f materials/session-55-root.pem ${L7VSD_CONF_DIR}/sslproxy/root.pem
...
RET=`wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-55-newcert-cl.pem --private-key=./materials/session-55-newkey-cl.pem --no-check-certificate`
EXPECT="${RealServer1}"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-55-newcert-cl.pem --private-key=./materials/session-55-newkey-cl.pem --no-check-certificate"
        exit 1
fi

...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-57-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
\cp -f materials/session-57-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
RET=`wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-57-newcert.pem --private-key=./materials/session-57-newkey.pem --no-check-certificate`
EXPECT="${RealServer1}"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-57-newcert.pem --private-key=./materials/session-57-newkey.pem --no-check-certificate"
        exit 1
fi

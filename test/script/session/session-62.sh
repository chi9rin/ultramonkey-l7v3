...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-62-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
\cp -f materials/session-62-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
RET=`wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-62-newcert.pem --private-key=./materials/session-62-newkey.pem --no-check-certificate`
if [ $? -eq 0 ]
then
        echo "Test failed: wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-62-newcert.pem --private-key=./materials/session-62-newkey.pem --no-check-certificate"
        exit 1
fi
...
RET=`cat ${L7VS_LOG_DIR}/l7vsd.log | egrep "\[ERROR\] l7vsd_session VSD40900113 ssl socket handshaking failed"`
if [ -z "${RET}" ]
then

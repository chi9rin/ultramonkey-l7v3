...
# set config file
. ${SET_DEFAULT_CONF}
\cp -f materials/session-56-l7vs.cf ${L7VSD_CONF_DIR}/l7vs.cf
\cp -f materials/session-56-sslproxy.target.cf ${L7VSD_CONF_DIR}/sslproxy/sslproxy.target.cf
...
RET=`wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-56-newcert.pem --private-key=./materials/session-56-newkey.pem`
if [ $? -eq 0 ]
then
        echo "Test failed: wget -O- https://127.0.0.1:40001/ --certificate=./materials/session-56-newcert.pem --private-key=./materials/session-56-newkey.pem "  
        exit 1
fi

if [ -n "${RET}" ]
then
        echo "Test failed: wget -qO- -t 1 -T 1 https://127.0.0.1:40001/ --certificate=newcert.pem --private-key=newkey.pem "
        exit 1
fi

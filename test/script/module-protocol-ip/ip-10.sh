...
usleep 100000
$L7VSADM -A -t 127.0.0.1:40001 -m ip --no-reschedule
if [ $? -ne 0 ]
then
	echo "Test failed: $L7VSADM -A -t 127.0.0.1:40001 -m ip --no-reschedule"
        exit 1
fi
...
if [ "${RET}" != "${RealServer1}" ]
then
	echo "Test failed: curl --retry 0 --interface 127.0.0.1 http://127.0.0.1:40001/"
	exit 1
fi
...
if [ $? -ne 0 ]
then
	echo "Test failed: $L7VSADM -d -t 127.0.0.1:40001 -m ip -r ${RealServer1_ADDR}:${RealServer1_PORT}"
        exit 1
fi

curl --retry 0 --interface 127.0.0.1 http://127.0.0.1:40001/
if [ $? -eq 0 ]
then
	echo "Test failed: curl --retry 0 --interface 127.0.0.1 http://127.0.0.1:40001/"
	exit 1

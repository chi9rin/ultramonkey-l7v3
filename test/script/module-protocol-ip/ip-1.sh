...
$L7VSADM -a -t 127.0.0.1:40001 -m ip -r ${RealServer3_ADDR}:${RealServer3_PORT}
if [ $? -ne 0 ]
then
	echo "Test failed: $L7VSADM -a -t 127.0.0.1:40001 -m ip -r ${RealServer3_ADDR}:${RealServer3_PORT}"
        exit 1
fi
...
RET=`curl --retry 0 --interface 127.0.0.2 http://127.0.0.1:40001/`
if [ "${RET}" != "${RealServer2}" ]
then
	echo "curl --retry 0 --interface 127.0.0.2 http://127.0.0.1:40001/"
	exit 1
fi
...
RET=`curl --retry 0 --interface 127.0.0.3 http://127.0.0.1:40001/`
if [ "${RET}" != "${RealServer3}" ]
then
	echo "curl --retry 0 --interface 127.0.0.3 http://127.0.0.1:40001/"
	exit 1
fi

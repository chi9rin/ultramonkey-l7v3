...
#Check Return Value
RET=`snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsScheduleModuleName.1`

EXPECT="ULTRAMONKEY-L7-MIB::l7vsVsScheduleModuleName.1 = STRING: rr"

if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsScheduleModuleName.1"
        exit 1
fi

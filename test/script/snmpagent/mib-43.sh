...
#Check Return Value
RET=`snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsReplicationMode.0`
echo "$RET"
EXPECT="ULTRAMONKEY-L7-MIB::l7vsReplicationMode.0 = INTEGER: single(1)"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsReplicationMode.0"
        exit 1
fi

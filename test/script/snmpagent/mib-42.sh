...
#Check Return Value
RET=`snmpget -v 2c -c public 127.0.0.1 ULTRAMONKEY-L7-MIB::l7vsRsInActiveConn.1`
echo "$RET"
EXPECT="ULTRAMONKEY-L7-MIB::l7vsRsInActiveConn.1 = INTEGER: 0"

if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: snmpget -v 2c -c public 127.0.0.1 ULTRAMONKEY-L7-MIB::l7vsRsInActiveConn.1"
        exit 1
fi

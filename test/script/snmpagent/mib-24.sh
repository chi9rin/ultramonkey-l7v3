...
#Check Return Value
RET=`snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsThroughputUpQoS.1`

EXPECT="ULTRAMONKEY-L7-MIB::l7vsVsThroughputUpQoS.1 = Counter64: 96 B/s"

if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsThroughputUpQoS.1"
        exit 1
fi

...
#Check Return Value
RET=`snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsScheduleModuleOptions.1`
echo "$RET"
EXPECT="ULTRAMONKEY-L7-MIB::l7vsVsScheduleModuleOptions.1 = STRING:  "

if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsScheduleModuleOptions.1"
        exit 1
fi

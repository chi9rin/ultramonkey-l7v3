...
#Add Virtual Service
$L7VSADM -A -t 127.0.0.1:40001 -m ip -Q 1M
if [ $? -ne 0 ]
then
...
#Check Traplog
RET=`cat  ${L7VS_LOG_DIR}/snmpagent-1-19-snmptrapd.log | grep -e  "SNMPv2-SMI::enterprises.32132.1.0.2.7 = STRING: \"[0-9]\{4\}/[0-9]\{2\}/[0-9]\{2\} [0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\},TRAP00020007,Warning: The up-throughput has exceeded the threshold of QoS warning."`

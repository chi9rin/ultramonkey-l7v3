...
#Check Return Value
EXPECT="ULTRAMONKEY-L7-MIB::l7vsVsThroughputDown.1 = Counter64: [0-9]* B/s"
RET=`snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsThroughputDown.1 | grep -e "${EXPECT}"`

if [ -z "${RET}" ]
then
        echo "Test failed: snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsThroughputDown.1"
        exit 1
fi

...
#Check Return Value
RET=`snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsNumber.0`

EXPECT="ULTRAMONKEY-L7-MIB::l7vsVsNumber.0 = INTEGER: 1"
if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsNumber.0"
        exit 1
fi

...
#Edit the Real service 
$L7VSADM -e -t 127.0.0.1:40001 -m sessionless -r 127.0.0.1:50001 -w 50
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -e -t 127.0.0.1:40001 -m sessionless -r 127.0.0.1:50001 -w 50"
        exit 1
fi
usleep 1000000
...
RET=`cat  ${L7VS_LOG_DIR}/snmpagent-1-19-snmptrapd.log | grep -e  "SNMPv2-SMI::enterprises.32132.1.0.2.5 = STRING: \"[0-9]\{4\}/[0-9]\{2\}/[0-9]\{2\} [0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\},TRAP00020005,The real server was changed.vs:127.0.0.1:40001,rs:127.0.0.1:50001"`
if [ -z "${RET}" ]
then

...
#Check Return Value
RET=`snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsSessionPoolCount.1`

EXPECT="ULTRAMONKEY-L7-MIB::l7vsVsSessionPoolCount.1 = INTEGER: 31"

if [ "${RET}" != "${EXPECT}" ]
then
        echo "Test failed: snmpget -v 2c -c public localhost ULTRAMONKEY-L7-MIB::l7vsVsSessionPoolCount.1"
        exit 1
fi

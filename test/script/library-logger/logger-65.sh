...
$L7VSADM -A -t 127.0.0.1:40001 -m sessionless -L 1 -a /var/log/l7vs/l7vs-access.log --ac-rotate-type "date" --ac-rotate-max-backup-index "10" --ac-rotate-rotation-timing "month" --ac-rotate-rotation-timing-value "`date -d '1 minute' +'%d %H:%M'`"
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -A -t 127.0.0.1:40001 -m sessionless -L 1 -a /var/log/l7vs/l7vs-access.log --ac-rotate-type date --ac-rotate-max-backup-index 10 --ac-rotate-rotation-timing month --ac-rotate-rotation-timing-value `date -d '1 minute' +'%d %H:%M'`"
        exit 1
fi

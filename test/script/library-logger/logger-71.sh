...
$L7VSADM -A -t 127.0.0.1:40001 -m sessionless -L 1 -a ${L7VS_LOG_DIR}/l7vs-access.log --ac-rotate-type "datesize" --ac-rotate-max-filesize "65535" --ac-rotate-max-backup-index "10" --ac-rotate-rotation-timing "year" --ac-rotate-rotation-timing-value "`date -d '1 month' +'%m/%d %H:%M'`"
if [ $? -ne 0 ]
then
        echo "Test failed: $L7VSADM -A -t 127.0.0.1:40001 -m sessionless -L 1 -a ${L7VS_LOG_DIR}/l7vs-access.log --ac-rotate-type datesize --ac-rotate-max-backup-index 10 --ac-rotate-rotation-timing year --ac-rotate-rotation-timing-value `date -d '1 month' +'%m/%d %H:%M'`"
        exit 1
fi

...
protocol_module_control::getInstance(){
	Logger logger( LOG_CAT_L7VSD_MODULE, 1, "protocol_module_control::getInstance", __FILE__, __LINE__ );
...
protocol_module_control::initialize( const std::string& infile_path ){
	Logger logger( LOG_CAT_L7VSD_MODULE, 1, "protocol_module_control::initialize", __FILE__, __LINE__ );
	if( &infile_path != NULL ){
...
protocol_module_control::finalize(){
	Logger logger( LOG_CAT_L7VSD_MODULE, 1, "protocol_module_control::finalize", __FILE__, __LINE__ );
}
...
protocol_module_control::load_module( const	std::string& modulename ){
	Logger logger( LOG_CAT_L7VSD_MODULE, 1, "protocol_module_control::load_module", __FILE__, __LINE__ );
...
protocol_module_control::unload_module( protocol_module_base* module_ptr ){
	Logger logger( LOG_CAT_L7VSD_MODULE, 1, "protocol_module_control::unload_module", __FILE__, __LINE__ );
	if( module_ptr == NULL ){
